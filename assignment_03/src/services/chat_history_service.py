...
class ChatHistoryService:
    ...
    def create_history(self, request: ChatHistoryCreateRequest) -> ChatHistoryRead:
        ...
        try:
            return self._repository.create(session_id=request.session_id,
            ...
            raise ConflictError(
                "같은 session_id 내에서 query_id는 유일해야 합니다.",
                details={"session_id": request.session_id, "query_id": request.query_id},
            ) from error
    ...
    def list_histories(
        ...
        if limit < 1:
            raise BadRequestError("limit는 1이상이어야 합니다.")
        if lt_query_id is None:
            return self._repository.list_by_session(session_id=session_id, limit=limit)
        else:
            return self._repository.list_before_query_id(session_id=session_id, query_id=lt_query_id, limit=limit)
        raise NotImplementedError("list_histories를 구현하세요.")
    ...
    def delete_all_histories(self) -> int:
        ...
        rowcounts = self._repository.delete_all()
        return rowcounts
        raise NotImplementedError("delete_all_histories를 구현하세요.")
