...
class ChatHistoryRepository:
    ...
    def list_before_query_id(
        ...
        logger.info(
            "Fetched histories session_id=%s lt_query_id=%s limit=%s count=%s query_ids=%s",
            session_id,
            query_id,
            limit,
            len(rows),
    ...
    def update(
        ...
        current = self.get(session_id=session_id, query_id=query_id)
        if current is None:
            return None

        next_user_message = user_message or current.user_message
        next_intent = intent or current.intent
        next_final_answer = final_answer or current.final_answer
        ...
        try:
            connection.execute(
                ...
                WHERE session_id = ? AND query_id = ?
                """,
                (next_user_message, next_intent, next_final_answer, session_id, query_id),
            )
            connection.commit()
        finally:
            connection.close()
        return self.get(session_id=session_id, query_id=query_id)
    ...
    def delete_all(self) -> int:
        ...
        connection = self._connect()
        try:
            cursor = connection.execute("DELETE FROM chat_histories")
            connection.commit()
            deleted_count = cursor.rowcount if cursor.rowcount >= 0 else 0
        finally:
            connection.close()

        logger.info("Deleted all chat histories count=%s", deleted_count)
